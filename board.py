    ...
    def draw(self, screen, images):
        ...
        for row in range(self.rows):
            for col in range(self.cols):
                ...
                # 繪製每個格子的邊框 (黑色格線)
                pygame.draw.rect(screen, (0, 0, 0), (x, y, self.tile_size, self.tile_size), 2)

        # 繪製棋盤外框 (黑色)
        board_width = self.cols * self.tile_size
        board_height = self.rows * self.tile_size
        board_x, board_y = 50, 300
        pygame.draw.rect(screen, (0, 0, 0), (board_x, board_y, board_width, board_height), 3)

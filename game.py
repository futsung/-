...
def main():
    ...
    # 初始化 Pygame
    pygame.init()
    pygame.mixer.init()  # 初始化音樂模組

    # 加載背景音樂
    music_path = os.path.join(os.path.dirname(__file__), "music", "bgm.mp3")
    ...
    # 配對音效設定
    match_sound_path = os.path.join(os.path.dirname(__file__), "music", "caraccident.mp3")
    if not os.path.exists(match_sound_path):
        print(f"Error: The sound file does not exist at {match_sound_path}")
        exit()
